...
export default class SurveyItem extends Component {
    ...
    state = {
        results: Array(survey.length).fill(false),
        verdict: undefined,
    };
    ...
    handleChange = index => (value) => {
        ...
        this.setState({ results: [ ...results.slice(0, index), value.findIndex(i => i.value) / 2, ...results.slice(index + 1) ] });
    };

    handleSubmit = () => {
        // Интерпретация результатов опросника IIEF:
        const test = (value) => {
        ...
        const verdict = test(this.state.results.reduce((res, i) => res + i, 0));
        ...
        return (
            <Row id="survey-item-page">
                ...
                <div className="survey-item__send">
                    <Button type="submit" name="submit" blue onClick={this.handleSubmit} disabled={results.some(x => x === false)}>Отправить</Button>
                </div>
